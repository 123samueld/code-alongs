document.addEventListener('DOMContentLoaded', () =>{
	...
	(function(index){
		//Add an onlick to each square in the grid
		squares[i].onclick = function(){
			//If the square below your current square is taken,
			//you can place on top of it
			if(squares[index + 7].classList.contains('taken')){
				...
				//If the square below isn't taken this one isn't available.
				else alert('Square not available, please choose another.')
				}
			}
...
function checkBoard(){
	...
	//Plug winning arrays into the squares
	for(let y = 0; y < winningArrays.length; y++){
		...
		if(
			...
			//If TRUE player two wins
			result.innerHTML = 'Player Two WINS!!!'
		}
	}
}

//Add an eventListener that will trigger the checkBoard function on click
